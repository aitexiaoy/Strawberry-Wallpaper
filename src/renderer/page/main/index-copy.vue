...
const { osType, imageSourceType, version } = utils
    ...
    methods: {
        ...
        eventInit() {
            ...
            this.$ipcRenderer.on('intervalTime', (event, arg) => {
                ...
                // 2小时 统计日活
                if (nowDate - statisticTimeFlag >= 2 * 60 * 60 * 1000) {
                    apiStatisticActive({
                ...
                // 7天 自动清除已下载
                if (nowDate - lastCleararnDownloadFilesTime >= (this.config.autoClearnDownloadFilesTime || 7) * 24 * 60 * 60 * 1000) {
                    this.$localStorage.setStore('lastCleararnDownloadFilesTime', nowDate)
                    // 删除默认文件下的所有内容
