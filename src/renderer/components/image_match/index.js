import elonresize from '../../assets/js/el-onresize.js'
    ...
    Vue.directive('imagematch', {
      bind(el, binding) {
        ...
        function _choponent_size_fn(e) {
          ...
          // console.log(parentWidth,parentHeight,el._childrenWidth,el._childrenHeight);

          // let children = el.firstElementChild;
          //宽度与高度自适应
          if (el._type == 'auto') {
            if (parentWidth / parentHeight > el._childrenWidth / el._childrenHeight) {
              children.style.cssText = `width:${parentWidth+'px'};height:auto;transform: translateY(${((el._childrenHeight-(parentHeight*el._childrenWidth/parentWidth))/2/(el._childrenWidth/parentWidth)*-1)+'px'});`
            } else {
              children.style.cssText = `width:auto;height:${parentHeight+'px'};transform: translateX(${((el._childrenWidth-(parentWidth*el._childrenHeight/parentHeight))/2/(el._childrenHeight/parentHeight)*-1)+'px'});`
            }
          }
          //宽度适应尺寸,高度自动
        ...
        children.onload = function () {
          ...
          // elonresize.$on(el, el._choponent_size);
